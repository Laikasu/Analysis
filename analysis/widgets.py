...
class Browser(QWidget):
    ...
    def update_plot(self):
        ...
        if self.peaks_fromfile is not None:
            self.sc.set_offsets(self.peaks_fromfile[:,frame[0], 0, 0,::-1])
...
class PeakFinder(Browser):
    ...
    #     frame = [slider.value()-1 for slider in self.sliders]
    #     # Calculate peaks
    #     drift = np.zeros((len(self.data.images[0]), 2), dtype=np.int16)
    ...
    #         # compute distance matrix
    #         D = cdist(self.old_peaks, new_peaks)
    ...
    #         drift[k] = np.mean([new_peaks[j] - self.peaks[i] for i, j in matches],axis=0).astype(np.int16)
    #         self.sliders[1].setValue(k+1)
    #         self.sc.set_offsets(self.peaks[:,::-1] + drift[k][::-1])
    #         self.old_peaks = self.peaks + drift[k]
    ...
    def apply_peaks(self):
        ...
        if i == len(self.data.images):
            ...
            n, N, _ = peaks.shape
            self.result = peaks.reshape(n, N, 1, 1, 2)
            self.result = self.result.swapaxes(0, 1).astype(np.uint16)
            self.close()
        
        self.sliders[0].setValue(i+1)
        displacement = self.calculate_drift()
